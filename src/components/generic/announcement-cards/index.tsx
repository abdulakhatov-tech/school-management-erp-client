...
const AnnouncementCards: React.FC<PropsI> = ({
  data,
  textRows = 2,
  loading,
}) => {
    ...
    <Card className='relative p-4'>
      ...
      <div className='flex flex-col gap-4 max-h-[400px] overflow-y-auto'>
        {loading ? (
          ...
          data?.map(({ _id, name, class: classN, date, description }) => (
            <Card
              key={_id}
            ...
            >
              <div className='grid grid-cols-[1fr_auto] gap-2'>
                ...
                <h4 className='bg-[rgba(195,235,250,0.3)] px-2 rounded-md text-sm center'>
                  {classN ? (
                    <Link
              ...
              <p
                className={`text-sm text-[#A1ADB4] line-clamp-${textRows}`}
              >
                {description}
              </p>
              <div className='flex items-center justify-end'>
                <time className="text-sm">
                  {formatDate(date, "dd MMM YYY")}
                </time>
              </div>
            </Card>
          ))
